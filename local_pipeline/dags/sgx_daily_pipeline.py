...
with DAG(
    ...
    ingestion_task = BashOperator(
        ...
        bash_command='python /opt/airflow/workspace/local_pipeline/01_Ingestion.py --date {{ ds }} --config /opt/airflow/workspace/local_pipeline/config/config.ini',
    )

    # Task 2: Xử lý ETL dữ liệu Ticks (Chạy song song)
    etl_tick_task = BashOperator(
        task_id='02_ETL_Tick_Data',
        bash_command='python /opt/airflow/workspace/local_pipeline/02_ETL_Tick_Data.py --date {{ ds }} --config /opt/airflow/workspace/local_pipeline/config/config.ini',
    )

    # Task 3: Xử lý ETL dữ liệu Hủy lệnh (Chạy song song)
    etl_trade_cancel_task = BashOperator(
        task_id='02_ETL_Trade_Cancel',
        bash_command='python /opt/airflow/workspace/local_pipeline/02_ETL_Trade_Cancel.py --date {{ ds }} --config /opt/airflow/workspace/local_pipeline/config/config.ini',
    )

    # Task 4: Bảo trì, tối ưu hóa các Delta tables
    maintenance_task = BashOperator(
        task_id='04_Maintenance',
        bash_command='python /opt/airflow/workspace/local_pipeline/04_Maintenance.py',
    )
